...
export default async function CheckInPage({
  ...
  return <div className="form-page"><div className="page-heading-row"><div><p className="eyebrow">Paid student intake</p><h1>Allocate room</h1><p>The initial payment is recorded. Complete the intake by assigning a specific room number.</p></div></div><CheckInForm allowCustomRent={["OWNER", "ADMIN"].includes(session.role)} currentRent={Number(intakePayment.charge.amount)} paymentId={intakePayment.id} rooms={roomOptions} selectedSemester={{ id: intakePayment.charge.semester.id, label: intakePayment.charge.semester.name }} selectedStudent={{ id: intakePayment.student.id, label: intakePayment.student.fullName }} semesterEnd={intakePayment.charge.semester.endDate.toISOString().slice(0, 10)} semesterStart={intakePayment.charge.semester.startDate.toISOString().slice(0, 10)} semesters={[]} students={[]} /></div>;
}
