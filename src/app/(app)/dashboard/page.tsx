...
export default async function DashboardPage() {
  ...
  const initialGreeting = kenyaHour < 12 ? "Good morning" : kenyaHour < 18 ? "Good afternoon" : "Good evening";
  ...
  const [rooms, students, charges, recentPayments] = await Promise.all([
    db.room.findMany({ where: { organizationId: session.organizationId }, include: { roomType: true, occupancies: { where: { status: "ACTIVE" }, include: { student: true } }, breakReservations: { where: { status: "RESERVED_FREE" }, include: { student: true } } }, orderBy: { number: "asc" } }),
