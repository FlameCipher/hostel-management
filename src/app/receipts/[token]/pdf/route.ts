...
export async function GET(
  ...
  const sharedReceipt = await verifyReceiptShareToken(token);
  if (!sharedReceipt) return new Response("This receipt link is invalid or has expired.", { status: 401 });

  const payment = await db.payment.findFirst({
    where: {
      id: sharedReceipt.paymentId,
      organizationId: sharedReceipt.organizationId,
    },
    include: {
      organization: true,
      student: true,
