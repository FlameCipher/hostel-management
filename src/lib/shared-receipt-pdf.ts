...
export async function sharedReceiptPdfResponse(paymentId: string, organizationId: string) {
  ...
  const pdf = await generateReceiptPdf({
    ...
    issuedAt: payment.issuedAt.toLocaleString("en-KE", { timeZone: "Africa/Nairobi" }),
    reprintCount: payment.reprintCount,
    verificationUrl: payment.securityReference ? `${process.env.NEXT_PUBLIC_APP_URL ?? ""}/verify-receipt/${encodeURIComponent(payment.securityReference)}` : null,
